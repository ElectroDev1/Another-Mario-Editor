/// @description scr_draw_message(x,y,string,opt1,opt2...)
/// @param x
/// @param y
/// @param string
/// @param opt1
/// @param opt2...
function scr_draw_message() {
	...
	*/
	...
	for(var a=0; a<array_length_1d(arr); a++){
	    ...
	    ds_list_add(list, string_height(string_hash_to_newline(arr[a])) );
	}
	...
	for(var a=0; a<array_length_1d(arr); a++){
	    messagewidth += string_width(string_hash_to_newline(arr[a]))/2+buttonpadding*2;
	}
	messagewidth+=buttonpadding*(array_length_1d(arr)-1);

	if(messagewidth<string_width(string_hash_to_newline(message))/2){ messagewidth=string_width(string_hash_to_newline(message))/2; }



	//Get the height
	var messageheight= string_height(string_hash_to_newline(message))/2 + buttonpadding*2 
	...
	draw_text_transformed(X/2,Y/2-messageheight/2+messagepadding,string_hash_to_newline(message),0.5,0.5,0);
	...
	if(array_length_1d(arr)>2){ 
	 ...
	 var offs=string_width(string_hash_to_newline(arr[0]))/4 + (Right-Left)/(array_length_1d(arr)/2+0.75) +buttonpadding*2;
 
	}else{

	  var offs=string_width(string_hash_to_newline(arr[0]))/4 + (Right-Left)/2 +buttonpadding*2;

	}

	for(var a=0; a<array_length_1d(arr); a++){
	    if(a>0){offs+=string_width(string_hash_to_newline(arr[a-1]))/4+string_width(string_hash_to_newline(arr[a]))/4+buttonpadding*3;}
	    ...
	    draw_set_colour(c_white);
	    draw_rectangle(
	    MX-string_width(string_hash_to_newline(arr[a]))/4-buttonpadding/2,MY-string_height(string_hash_to_newline(arr[a]))/4-buttonpadding/2,
	       MX+string_width(string_hash_to_newline(arr[a]))/4+buttonpadding/2,MY+string_height(string_hash_to_newline(arr[a]))/4+buttonpadding/2,0);
    
	    draw_set_colour(c_black);
	    draw_text_transformed(MX,MY,string_hash_to_newline(arr[a]),0.5,0.5,0);
